...
let adminBrowseHistory = []; // for browsing user's folder structure
...
async function addDomain() {
    const input = document.getElementById("domainInput"); const domain = input.value.trim();
    ...
    try {
        ...
        if (res.ok) { showToast("Domain added", "success"); input.value = ""; fetchDomains(); } else showToast(data.message || "Failed", "error");
...
async function fetchDomains() {
    try {
        ...
        const data = await res.json();
        const list = document.getElementById("domainList"); list.innerHTML = "";
        ...
        data.forEach(d => {
            ...
            deleteBtn.onclick = () => openConfirmModal(`Delete domain "${d.name}"?`, async () => { const r = await fetch(`${API}/domain/${d.id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } }); if (r.ok) { showToast("Domain deleted", "success"); fetchDomains(); } });
...
async function addUser() {
    const input = document.getElementById("userEmail"); const email = input.value.trim();
    ...
    try {
        ...
        const data = await res.json();
        if (res.ok) { showToast("User added", "success"); input.value = ""; fetchUsers(); } else showToast(data.message || "Failed", "error");
    } catch (err) { showToast("Connection error", "error"); }
}
...
async function fetchUsers() {
    try {
        ...
        data.forEach(u => {
            ...
            const actions = document.createElement("div"); actions.className = "item-actions";
            const deleteBtn = document.createElement("button"); deleteBtn.className = "btn btn-danger btn-sm"; deleteBtn.textContent = "Delete";
            deleteBtn.onclick = () => openConfirmModal(`Remove user "${u.email}"?`, async () => { const r = await fetch(`${API}/user/${u.id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } }); if (r.ok) { showToast("User removed", "success"); fetchUsers(); } });
...
async function fetchRegisteredUsers() {
    try {
        ...
        const tbody = document.getElementById('userAnalyticsBody'); tbody.innerHTML = '';
        if (users.length === 0) { tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:var(--text-muted);padding:32px;">No registered users</td></tr>'; return; }
        users.forEach(u => {
            const tr = document.createElement('tr');
            const pct = Math.min((u.storage_used / (500 * 1048576)) * 100, 100);
            tr.innerHTML = `
...
function renderDetailList(containerId, items, mode) {
    ...
    if (!items || items.length === 0) { el.innerHTML = `<div class="detail-empty">No ${mode === 'owned' ? 'files or folders' : 'shared files'} found</div>`; return; }
    el.innerHTML = '';
    items.forEach(item => {
        ...
        if (mode === 'owned') {
            ...
            const del = document.createElement('button'); del.className = 'admin-del-btn'; del.textContent = 'Delete'; del.title = 'Delete as admin';
            del.onclick = () => openConfirmModal(`Admin delete "${item.name}"?`, async () => {
                ...
                if (r.ok) { showToast('Deleted', 'success'); openUserDetail(currentDetailUserId); } else showToast('Failed', 'error');
            });
            row.appendChild(del);
        }
...
// ════════════════════════════════════
//  ADMIN FILE MANAGEMENT (Manage Files tab)
// ════════════════════════════════════
async function adminBrowseFiles(parentId) {
    ...
    try {
        ...
        if (files.length === 0) {
            el.innerHTML = '<div class="detail-empty">No files or folders here</div>';
            return;
        }
        ...
        files.forEach(item => {
            ...
            if (item.type === 'folder') {
                ...
                name.onclick = () => {
                    adminBrowseHistory.push({ id: item.id, name: item.name });
                    adminBrowseFiles(item.id);
                };
            }

            const meta = document.createElement('div'); meta.className = 'detail-file-meta';
            meta.textContent = item.type === 'file' ? formatSize(item.size) : 'Folder';
            ...
            del.onclick = () => openConfirmModal(`Delete "${item.name}" from this user?`, async () => {
                ...
                if (r.ok) {
                    ...
                    const currentParent = adminBrowseHistory.length > 0 ? adminBrowseHistory[adminBrowseHistory.length - 1].id : null;
                    adminBrowseFiles(currentParent);
...
function updateAdminBrowseBc() {
    ...
    const root = document.createElement('a'); root.textContent = 'Root'; root.style.cursor = 'pointer'; root.style.color = 'var(--accent)'; root.style.fontSize = '12px'; root.style.textDecoration = 'none';
    root.onclick = () => { adminBrowseHistory = []; adminBrowseFiles(null); };
    bc.appendChild(root);

    adminBrowseHistory.forEach((f, i) => {
        const sep = document.createElement('span'); sep.textContent = ' › '; sep.style.color = 'var(--text-hint)'; sep.style.fontSize = '12px';
        bc.appendChild(sep);
        const link = document.createElement('a'); link.textContent = f.name; link.style.cursor = 'pointer'; link.style.color = 'var(--accent)'; link.style.fontSize = '12px'; link.style.textDecoration = 'none';
...
function switchDetailTab(tab) {
    const tabs = ['owned', 'shared-with', 'shared-by', 'manage'];
