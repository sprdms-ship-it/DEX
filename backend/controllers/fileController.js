...
exports.uploadFile = async (req, res) => {
    try {
        ...
        if (parent_id) {
            ...
            if (!parentFolder) {
                return res.status(400).json({
                    message: 'Parent folder not found or not accessible'
                });
            }
        }
        ...
        // 👉 GCS path (based on your senior's doc)
        const gcsPath = `projects/Aritra/users/${user.id}/uploads/${fileName}`;
...
exports.createFolder = async (req, res) => {
    try {
        ...
        await db.runAsync(
            `INSERT INTO files (id, name, type, parent_id, owner_id)
             VALUES (?, ?, ?, ?, ?)`,
...
// ─── GET FILES (list contents of a folder or root) ───
exports.getFiles = async (req, res) => {
    try {
        ...
        // Get files shared directly with this user at this level
        const sharedFiles = await db.allAsync(
            `SELECT f.* FROM files f
             INNER JOIN permissions p ON p.file_id = f.id
             WHERE p.user_id = ? AND f.parent_id IS ?
             ORDER BY f.type DESC, f.name ASC`,
            [user.id, parent_id || null]
        );

        // Get files shared via domain
        ...
        if (userDomain) {
            domainFiles = await db.allAsync(
                `SELECT f.* FROM files f
                 INNER JOIN permissions p ON p.file_id = f.id
                 WHERE p.domain = ? AND f.parent_id IS ?
                 ORDER BY f.type DESC, f.name ASC`,
...
exports.shareWithUser = async (req, res) => {
    try {
        ...
        if (!file_id || !target_email || !role) {
            return res.status(400).json({
                message: 'file_id, target_email, and role are required'
            });
        }
        ...
        if (!targetUser) {
            return res.status(404).json({
                message: 'User not registered. Cannot share file.'
            });
        }
        ...
        const existing = await db.getAsync(
            ...
            [file_id, targetUser.id]
        );

        if (existing) {
            await db.runAsync(
                `UPDATE permissions SET role = ? WHERE file_id = ? AND user_id = ?`,
                [role, file_id, targetUser.id]
            );
        } else {
            await db.runAsync(
                `INSERT INTO permissions (id, file_id, user_id, role)
                 VALUES (?, ?, ?, ?)`,
                [uuidv4(), file_id, targetUser.id, role]
            );
        }
...
exports.shareWithDomain = async (req, res) => {
    try {
        ...
        if (!file_id || !domain || !role) {
            return res.status(400).json({
                message: 'file_id, domain, and role are required'
            });
        }
        ...
        const existingDomainUser = await db.getAsync(
        ...
        if (existing) {
            await db.runAsync(
                `UPDATE permissions SET role = ? WHERE file_id = ? AND domain = ?`,
                [role, file_id, domain]
            );
        ...
        await db.runAsync(
            `INSERT INTO permissions (id, file_id, domain, role) VALUES (?, ?, ?, ?)`,
            [uuidv4(), file_id, domain, role]
        );
...
exports.createShareLink = async (req, res) => {
    try {
        ...
        if (!file_id || !access_type) {
            return res.status(400).json({ message: 'file_id and access_type required' });
        }
        ...
        await db.runAsync(
            `INSERT INTO share_links (id, file_id, token, access_type, expires_at)
             VALUES (?, ?, ?, ?, ?)`,
        ...
        res.json({
            ...
            link: `${req.protocol}://${req.get('host')}/api/files/public/${token}`,
...
exports.accessSharedFile = async (req, res) => {
    try {
        ...
        const file = await db.getAsync(
            `SELECT id, name, type, size FROM files WHERE id = ?`, [link.file_id]
        );
...
exports.moveFile = async (req, res) => {
    try {
        ...
        if (new_parent_id) {
            let currentParent = new_parent_id;
            while (currentParent) {
                if (currentParent === file_id) {
                    return res.status(400).json({ message: 'Cannot move a folder into itself or its subfolder' });
                }
                const parent = await db.getAsync(`SELECT parent_id FROM files WHERE id = ?`, [currentParent]);
...
async function sendShareEmail(toEmail, fileName, sharedBy, fileId) {
    try {
        ...
        const iconColors = {
            ...
            ppt: '#E65100', pptx: '#E65100', txt: '#546E7A',
            jpg: '#00897B', jpeg: '#00897B', png: '#00897B', gif: '#00897B',
            mp4: '#6A1B9A', mp3: '#6A1B9A', zip: '#F57F17', rar: '#F57F17'
        };
...
<td style="padding:36px 36px 20px;">
        ...
        <td style="padding:20px 24px;">
            ...
            <tr>
                <td width="48" style="vertical-align:top;">
                    <div style="width:44px; height:44px; background-color:${iconColor}15; border-radius:10px; text-align:center; line-height:44px;">
                        <span style="font-size:22px; color:${iconColor};">&#128196;</span>
                    </div>
                </td>
                <td style="padding-left:14px; vertical-align:middle;">
                    <p style="margin:0 0 4px; font-size:15px; font-weight:600; color:#111827; word-break:break-word;">${fileName}</p>
                    <p style="margin:0; font-size:13px; color:#9ca3af;">.${ext.toUpperCase()} file &bull; Shared as viewer</p>
                </td>
            </tr>
            </table>
        </td>
    ...
    <tr><td align="center">
        <a href="${link}" style="display:inline-block; padding:14px 40px; background-color:#0e1520; color:#ffffff; font-size:15px; font-weight:600; text-decoration:none; border-radius:10px; letter-spacing:0.3px;">
            Open File
        </a>
    </td></tr>
    </table>
    ...
    <p style="margin:20px 0 0; font-size:12px; color:#9ca3af; text-align:center;">
        You'll need to sign in to access this file
    </p>
...
<td style="padding:20px 36px 28px; text-align:center;">
    ...
    <p style="margin:0; font-size:11px; color:#d1d5db;">This is an automated notification from ONE SPR FTP DEX. Do not reply to this email.</p>
</td>
</tr>
